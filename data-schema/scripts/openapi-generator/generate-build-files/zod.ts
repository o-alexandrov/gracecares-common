...
/**
 * @notes
 * this is a copy from orval/zod (before v4 support)
 */
import * as preprocessFns from "@gracecares-ai/data-schema/build/validators.preprocess"
import {
  camel,
  ClientBuilder,
...
const preprocessFunctions = Object.keys(preprocessFns)
...
const ZOD_DEPENDENCIES: GeneratorDependency[] = [
  ...
  {
    exports: preprocessFunctions.map((fnName) => ({
      name: fnName,
      values: true,
    })),
    dependency: `./validators.preprocess`,
  },
]
...
export const generateZodValidationSchemaDefinition = (
  ...
  if ((schema as any).preprocess) {
    const preprocessType = (schema as any).preprocess
    if (
      typeof preprocessType === `string` &&
      preprocessFunctions.includes(
        preprocessType as (typeof preprocessFunctions)[number],
      )
    ) {
      // Wrap the entire schema with preprocessing
      return {
        functions: [
          [
            `preprocess`,
            {
              type: preprocessType,
              schema: { functions, consts: uniq(consts) },
            },
          ],
        ],
        consts: uniq(consts),
...
${Object.entries(args)
  ...
  .join(`,\n`)}
    ...
    if (fn === `preprocess`) {
      const preprocessType = args.type
      const schema = (args.schema as ZodValidationSchemaDefinition).functions
        .map(parseProperty)
        .join(``)
      const wrappedSchema = `${schema.startsWith(`.`) ? `zod` : ``}${schema}`
      return `zod.preprocess(${preprocessType}, ${wrappedSchema})`
    }
...
export const zodClientBuilder: ClientGeneratorsBuilder = {
  client: generateZod,
